...
A가 기업별 재무비율 CSV를 병합하여 단일 분석용 데이터셋 `merged_dataset.csv`를 생성할 때 따라야 하는 스키마와 정규화 규칙을 정의한다.

이 문서는 현재 구현된 비율 산출 로직([src/etl/ratio_calculator.py](/Users/hann/Project/kwoss/kw0ss_project/src/etl/ratio_calculator.py))과 raw 구조 점검 문서([docs/raw_schema_check.md](/Users/hann/Project/kwoss/kw0ss_project/docs/raw_schema_check.md))를 기준으로 작성한다.

## 산출물

- 파일 형식: CSV
- 인코딩: UTF-8
- 파일명: `merged_dataset.csv`
- 권장 저장 위치: `data/merged_dataset.csv`
...
`merged_dataset.csv`는 아래 입력을 병합해 생성한다.

1. 기업별 재무비율 CSV
   - 예: `data/output/{gics_sector}/{stock_code}_{year}.csv`
   - 현재 컬럼: `stock_code`, `corp_name`, `year`, `quarter`, `label` + 재무비율/재무항목 컬럼
2. 기업 메타데이터
   - 최소 필요 컬럼: `stock_code`, `gics_sector`
   - 선택 컬럼: `corp_name`

현재 비율 CSV에는 `gics_sector`가 포함되어 있지 않으므로 병합 단계에서 별도 메타데이터를 통해 반드시 보강해야 한다.
...
병합 단계에서는 원칙적으로 재무비율을 재계산하지 않고, 기업별 비율 CSV 값을 그대로 적재한다.

다만 검증이 필요할 경우 계산식 기준은 아래 문서를 따른다.

- raw 구조 및 계정 매핑: [docs/raw_schema_check.md](/Users/hann/Project/kwoss/kw0ss_project/docs/raw_schema_check.md)
- 실제 산출 로직: [src/etl/ratio_calculator.py](/Users/hann/Project/kwoss/kw0ss_project/src/etl/ratio_calculator.py)

현재 확인된 구현상 주의사항은 다음과 같다.

- `유형자산회전율`은 현재 `총자본회전율`과 동일한 계산값으로 생성된다.
- `부채비율`은 일반적 의미의 `부채/자본`이 아니라 `자산/자본 × 100` 정의를 따른다.
- `비유동자산장기적합률`, `무형자산상각비`, `유형자산상각비`, `감가상각비`는 현재 구현상 빈 값일 수 있다.
