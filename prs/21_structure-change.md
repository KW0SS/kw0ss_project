...
- DART 원본 JSON 수집 결과를 바로 모델링에 쓰기 어려워, 재무비율 생성 이후의 전처리 규칙과 학습 실험 구조를 코드로 고정할 필요가 있었다.
- 특히 상장폐지 예측 데이터는 H-horizon별 라벨링, 연도 기반 split, 극심한 클래스 불균형 대응 기준이 먼저 정해져야 이후 실험 결과를 비교 가능하게 만들 수 있었다.
- 기존 브랜치에 흩어져 있던 EDA/데이터 계약/전처리/모델링 작업을 이번 브랜치에서 하나의 분석 파이프라인으로 연결했다.

**주요 변경 사항**

- raw JSON에서 표준 계정 매핑과 재무비율 계산을 거쳐 CSV를 만드는 전처리 경로를 정리하고, `combined_raw.csv`를 기준으로 H6~H24 rolling label, time split, 결측 보간, clipping을 적용한 학습용 데이터셋을 생성하도록 확장했다.
- 분석 단계에서는 데이터 계약서, 스키마 점검 문서, EDA 템플릿 노트북, 재사용 가능한 분석 유틸을 추가해 데이터 수신 직후 구조 확인과 탐색 분석을 반복 가능한 형태로 만들었다.
- 모델링 단계에서는 PR-AUC 중심 평가 프레임워크, threshold 최적화, 불균형 전략 비교 실험, tree-based 모델 학습 모듈, 실험 간 비교 스크립트를 추가해 baseline 실험을 재현 가능하게 구성했다.
...
- 수집 이후 데이터 표준화, 분석, 학습, 결과 비교까지의 후속 작업이 문서가 아니라 코드와 산출물 중심으로 전환되어, 이후 실험은 동일 split/전처리 규칙 위에서 반복 실행할 수 있다.
- 전처리 결과물의 디렉터리 구조가 `processed/H{horizon}/train|valid|test|meta.json` 형태로 고정되면서, 모델 학습 코드와 실험 리포트가 동일한 입력 계약을 공유하게 된다.
