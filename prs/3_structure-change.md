...
- `eda.utils`에 CSV 로드, 컬럼 분류, 기본 통계, 결측 분석, 단변량/label별 비교, 상관관계, IQR 기반 이상치 탐지 및 읽기 쉬운 이상치 판정표 렌더링 함수를 추가했다.
- `eda/eda_template.ipynb`와 실행 HTML을 추가해 `clean_data_no_macro.csv` 기준으로 기본 통계, 결측, 라벨 분포, 단변량 분포, 상관관계, 이상치 요약, baseline 모델 결과를 한 번에 확인할 수 있게 했다.
...
### 영향 범위
- 기존 수집/S3 조회 코드의 런타임 동작을 직접 변경하기보다, 분석 단계에서 사용할 신규 `eda`, `src/baseline`, `docs` 산출물을 추가하는 영향이 크다.
...
**src/**
  ...
  - `src/analysis/utils.py` (추가, 호환용 shim)
  - `src/baseline/__init__.py` (추가)
  - `src/baseline/run_baseline.py` (추가)
**eda/**
  - `eda/__init__.py` (추가)
  - `eda/utils.py` (추가)
  - `eda/eda_template.executed.html` (추가)
  - `eda/eda_template.ipynb` (추가)
**root/**
  - `README.md` (추가)
  - `requirements.txt` (수정)
**other/**
